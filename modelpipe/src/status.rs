...
pub enum PipeStatus {
    ...
    /// follows.
    ///
    /// **A transport failure is not one of the ways to get here.** Nothing
    /// gives up on reaching a peer: an unreachable one is
    ...
    /// `Copy`; the diagnostic accessor that answers *which* of those it was
    /// is `close_reason`, on
